...
__device__ inline bool is_bad(float x) {
    return !(x == x) || isinf(x);   // x!=x  °Ê NaN
}
    ...
    float* dL_ds_val)
{
    ...
    bool prev_vox_valid = false;  // Whether valid 'previous voxel' information exists
    float3 prev_vox_c;            // Previous voxel's center
    float prev_vox_l;             // Previous voxel's length
    float prev_geo_params[8];     // Previous voxel's geometry parameters (SDF)
    float3 prev_vox_rgb;          // Previous voxel's rgb
    int prev_vox_id;
    ...
    for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
    {
        ...
        for (int jj = 0; !done && jj <= j_lst_top; jj++)
        {
            ...
            float dL_dgeo_params_j[8]    = {0.f};
            float dL_dgeo_params_prev[8] = {0.f};

            float alpha = 0.f;
            float d_alpha_d_entry = 0.f;
            float d_alpha_d_exit  = 0.f;
            float sdf_entry = 0.f, sdf_exit = 0.f;
            float interp_w_middle[8], interp_w_prev_middle[8];
            float prev_phi_middle = 0.f, phi_middle = 0.f;
            float sdf_middle = 0.f, prev_sdf_middle = 0.f;
            float portion = 0.f;
            float3 vox_rgb = {0.f,0.f,0.f};
            ...
            {
                if(prev_vox_valid){
                    //const int next_j = j_lst[jj - 1];
                    //const float3 next_vox_c = collected_vox_c[next_j];
                    //const float next_vox_l = collected_vox_l[next_j];
                    const float2 prev_ab = ray_aabb(prev_vox_c, prev_vox_l, ro, rd_inv);
                    // Use a tolerance to treat near-equal boundaries as touching due to FP error
                    const float t_touch_tol = 1e-5f * (1.f + fabsf(b));
                    if (prev_ab.x > b + t_touch_tol) {
                        prev_vox_valid=true;
                        prev_vox_c= vox_c;
                        prev_vox_l= vox_l;
                        prev_vox_rgb = collected_rgb[j];
                        for (int k=0; k<8; ++k)
                            prev_geo_params[k]= geo_params[k];
                        prev_vox_id = collected_vox_id[j];

                        //printf("next_ab.x: %f, b: %f\n", next_ab.x, b);
                        continue;
                    }
                    //float2 next_ab_adj = next_ab;
                    //if (fabsf(next_ab.x - b) <= t_touch_tol) {
                    //    next_ab_adj.x = b; // snap to current exit to avoid tiny gaps/overlaps
                    //}
                    float3 p_middle = ro + (a+b)*0.5f * rd;
                    float3 qt_middle = (p_middle - (vox_c - 0.5f * vox_l))*vox_l_inv;
                    //next voxel entry
                    float prev_vox_l_inv = 1.f / prev_vox_l;
                    float3 prev_p_middle = ro + (prev_ab.x + prev_ab.y) * 0.5f * rd;
                    float3 prev_qt_middle = (prev_p_middle - (prev_vox_c - 0.5f * prev_vox_l))*prev_vox_l_inv;
                    tri_interp_weight(qt_middle, interp_w_middle);
                    
                    for (int iii=0; iii<8; ++iii)
                        sdf_middle += geo_params[iii] * interp_w_middle[iii];
                    tri_interp_weight(prev_qt_middle, interp_w_prev_middle);
                    for (int iii=0; iii<8; ++iii)
                        prev_sdf_middle += prev_geo_params[iii] * interp_w_prev_middle[iii];
                    phi_middle = 1.f / (1.f + expf(-s_val[0] * sdf_middle));
                    prev_phi_middle = 1.f / (1.f + expf(-s_val[0] * prev_sdf_middle));
                    alpha = min(MAX_ALPHA, fmaxf((phi_middle - prev_phi_middle) / (phi_middle+1e-6f), 0.f));
                    if (alpha < MIN_ALPHA)
                        continue;
                    vox_rgb = collected_rgb[j];
                    float3 p_exit = ro + b * rd;
                    portion = sqrtf(dot(prev_p_middle - p_exit, prev_p_middle - p_exit)) / (sqrtf(dot(prev_p_middle - p_middle, prev_p_middle - p_middle)) + 1e-6f);
                    float3 blended_rgb = portion * prev_vox_rgb + (1-portion) * vox_rgb;
                    // reverse-order reconstruction to match forward weights
                    T = T / (1.f - alpha);
                    pt_w = alpha * T;
                    vox_id = collected_vox_id[j];
                    c = blended_rgb;
                }else{
                    prev_vox_valid=true;
                    prev_vox_c= vox_c;
            ...
            if (need_normal && density_mode == DENSITY_SDF && sdf0_t > 0.f) {
                ...
                if (sdf_in_voxel) {
                    ...
                    float3 N = grad * norm;  // forward?ùò normal
                    float3 dL_dN_sdf = T * dL_dN;  // pt_w?äî T * alpha, ?ù¥Í≤? normal loss?óê ?ï¥?ãπ

                    // Chain rule: dL/dg = dL/dN ?ãÖ dN/dg = (I - NN·µ?) * dL/dN * dgrad/dg
            ...
            **************************/
            if(density_mode == DENSITY_SDF)
            {   
                float phi_middle_prime      = s_val[0] * phi_middle     * (1.f - phi_middle);
                float prev_phi_middle_prime = s_val[0] * prev_phi_middle* (1.f - prev_phi_middle);

                // alpha = (phi_middle - next_phi_middle) / phi_middle
                float d_alpha_d_middle      = (1.f - alpha) / (phi_middle + 1e-6f) * phi_middle_prime;
                float d_alpha_d_prev_middle = -(1.f / (phi_middle + 1e-6f)) * prev_phi_middle_prime;

                // --- (2) Accumulate gradient to SDF geo params of j and next_j separately
                
                for (int i = 0; i < 8; ++i) {
                    dL_dgeo_params_j[i]    += d_alpha_d_middle      * interp_w_middle[i]      * dL_dalpha;
                    dL_dgeo_params_prev[i] += d_alpha_d_prev_middle * interp_w_prev_middle[i] * dL_dalpha;
                }

                // derivative wrt s_val (temperature)
                if (dL_ds_val != nullptr) {
                    float d_phi_middle_ds      = phi_middle     * (1.f - phi_middle)     * sdf_middle;
                    float d_prev_phi_middle_ds = prev_phi_middle* (1.f - prev_phi_middle)* prev_sdf_middle;
                    float d_alpha_ds_val =
                        ((d_phi_middle_ds * (1.f - alpha)) - d_prev_phi_middle_ds) / (phi_middle + 1e-6f);
                    atomicAdd(&dL_ds_val[0], dL_dalpha * d_alpha_ds_val);
                }
            }
            else{
            ...
            **************************/
            if(density_mode == DENSITY_SDF){
                float dL_drgb_j[3]    = {pt_w * (1.f-portion)      * dL_dpix.x,
                                        pt_w * (1.f-portion)      * dL_dpix.y,
                                        pt_w * (1.f-portion)      * dL_dpix.z};
                float dL_drgb_prev[3] = {pt_w * portion * dL_dpix.x,
                                        pt_w * portion * dL_dpix.y,
                                        pt_w * portion * dL_dpix.z};

                if (lambda_R_concen > 0) {
                    const float3 grad_R_concen_j    = weight_R_concen * pt_w * (1.f-portion)      * 2.f * (c    - gt_pix);
                    const float3 grad_R_concen_prev = weight_R_concen * pt_w * portion * 2.f * (c - gt_pix);
                    dL_drgb_j[0]    += grad_R_concen_j.x;    dL_drgb_j[1]    += grad_R_concen_j.y;    dL_drgb_j[2]    += grad_R_concen_j.z;
                    dL_drgb_prev[0] += grad_R_concen_prev.x; dL_drgb_prev[1] += grad_R_concen_prev.y; dL_drgb_prev[2] += grad_R_concen_prev.z;
                }
                float grad_pack_j[12];
                for (int iii = 0; iii < 8; ++iii)
                    grad_pack_j[iii] = dL_dgeo_params_j[iii];
                grad_pack_j[8]  = dL_drgb_j[0];
                grad_pack_j[9]  = dL_drgb_j[1];
                grad_pack_j[10] = dL_drgb_j[2];
                grad_pack_j[11] = fabs(dL_dalpha * alpha * (1.f-portion) );

                for (int iii = 0; iii < 12; ++iii) {
                    if (is_bad(grad_pack_j[iii])) {
                        printf("NaN grad_pack_j: vox %d idx %d val %.6e  pt_w %.6e alpha %.6e portion %.6e\n",
                               vox_id, iii, grad_pack_j[iii], pt_w, alpha, (1.f-portion));
                    }
                }
                const int base_id = cg::this_grid().thread_rank();
                #pragma unroll
                for (int iii=0; iii<12; ++iii)
                    atomicAdd(dL_dvox + vox_id * 12 + (base_id+iii)%12, grad_pack_j[(base_id+iii)%12]);
                // --- (5) Pack and atomicAdd for voxel next_j
                float grad_pack_prev[12];
                for (int iii = 0; iii < 8; ++iii)
                    grad_pack_prev[iii] = dL_dgeo_params_prev[iii];
                grad_pack_prev[8]  = dL_drgb_prev[0];
                grad_pack_prev[9]  = dL_drgb_prev[1];
                grad_pack_prev[10] = dL_drgb_prev[2];
                grad_pack_prev[11] = fabs(dL_dalpha * alpha * portion);
                for (int iii = 0; iii < 12; ++iii) {
                    if (is_bad(grad_pack_prev[iii])) {
                        printf("NaN grad_pack_prev: next_vox %d idx %d val %.6e  pt_w %.6e alpha %.6e portion %.6e\n",
                               prev_vox_id, iii, grad_pack_prev[iii], pt_w, alpha, portion);
                    }
                }
            
                #pragma unroll
                for (int iii=0; iii<12; ++iii)
                    atomicAdd(dL_dvox + prev_vox_id * 12 + (base_id+iii)%12, grad_pack_prev[(base_id+iii)%12]);
                prev_vox_valid=true;
                prev_vox_c= vox_c;
                prev_vox_l= vox_l;
                prev_vox_rgb = collected_rgb[j];
    ...
    float* dL_ds_val)
{
    ...
    (density_mode == DENSITY_SDF) ?
        ...
        BwRendFunc(3, DENSITY_SDF); //Î¨¥Ï°∞Í±? ?ù¥?†áÍ≤?
