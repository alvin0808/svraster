    ...
    const float* __restrict__ s_val)
{
    ...
    for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
    {
        ...
        for (int j = 0; !done && j < end_j; j++)
        {
            ...
            j_lst_top += 1;
            j_lst[j_lst_top] = j; // list of voxel indexes that contribute to the pixel
        }
        ...
        {
            int j = j_lst[jj];
            const int vox_id = collected_vox_id[j]; // voxel id
            ...
            const float a = ab.x;
            const float b = ab.y;
            float2 prev_ab = {0.f, 0.f};
            float geo_params[8];
            for (int k=0; k<8; ++k)
                geo_params[k] = collected_geo_params[j*8 + k];
            //float2 next_ab = {0.f, 0.f};
            //float3 next_vox_c = {0.f, 0.f, 0.f};
            //float next_vox_l = 0.f;
            //float next_geo_params[8];
            ...
            if (density_mode == SDF_MODE){
                //find next voxel entry
                if(prev_vox_valid){
                    //const int next_j = j_lst[jj + 1];
                    //next_vox_c = collected_vox_c[next_j];
                    //next_vox_l = collected_vox_l[next_j];
                    prev_ab = ray_aabb(prev_vox_c, prev_vox_l, ro, rd_inv);
                    // Use a tolerance to treat near-equal boundaries as touching due to FP error
                    const float t_touch_tol = 1e-5f * (1.f + fabsf(prev_ab.y));
                    if (a > prev_ab.y + t_touch_tol) {
                        prev_vox_valid=true;
                        prev_vox_c= vox_c;
                        prev_vox_l= vox_l;
                    ...
                    tri_interp_weight(qt_middle, interp_w);
                    float sdf_middle = 0.f, prev_sdf_middle = 0.f;
                    for (int iii=0; iii<8; ++iii)
                        sdf_middle += geo_params[iii] * interp_w[iii];
                    tri_interp_weight(prev_qt_middle, interp_w);
                    
                    for (int iii=0; iii<8; ++iii)
                        prev_sdf_middle += prev_geo_params[iii] * interp_w[iii];
                    float phi_middle = 1.f / (1.f + expf(-s_val[0] * sdf_middle));
                    float prev_phi_middle = 1.f / (1.f + expf(-s_val[0] * prev_sdf_middle));
                    alpha = fmaxf((prev_phi_middle - phi_middle) / (prev_phi_middle+1e-6f), 0.f);

                    float t_mid      = 0.5f * (a + b);
                    float t_prev_mid = 0.5f * (prev_ab.x + prev_ab.y);
                    if (prev_sdf_middle > 0.f && sdf_middle < 0.f && !has_sdf0) {
                        float denom  = (prev_sdf_middle - sdf_middle) + 1e-6f;
                        float ratio  = (prev_sdf_middle) / denom; // 0~1
                        float t_sdf0 = t_prev_mid + ratio * (t_mid - t_prev_mid);
                        sdf0_t = t_sdf0;
                        has_sdf0 = true;
                    }
                    ...
                    // rgb accumulation
                    pt_w = T * alpha;
                    float3 vox_rgb = collected_rgb[j];
                    float3 prev_p_exit = ro + prev_ab.y * rd;
                    float portion = sqrtf(dot(p_middle - prev_p_exit, p_middle - prev_p_exit)) / (sqrtf(dot(p_middle - prev_p_middle, p_middle - prev_p_middle)) + 1e-6f);
                    float3 blended_rgb = portion * vox_rgb + (1-portion) * prev_vox_rgb;
                    C = C + pt_w * blended_rgb;
                ...
                float3 pt = ro + (a + 0.5f * step_sz) * rd; //sample center point
                float3 qt = (pt - (vox_c - 0.5f * vox_l)) * vox_l_inv; // sample Ïß??†ê?ùò voxel Ï¢åÌëú
            ...
            if (need_depth)
            {
                ...
                else if (n_samp == 2 && density_mode != SDF_MODE)
                {
                    ...
                    // sdf Î™®ÎìúÎ°úÎèÑ ?àò?†ï?ï¥?ïº?ê®
                }
                else if(density_mode != SDF_MODE)
                {
                    dval = alpha * 0.5f * (a + b);
                }
                else{
                    if(prev_vox_valid)
                        dval = alpha * 0.25f * (prev_ab.x + prev_ab.y+a+b);
                    else 
                        dval = 0;
                }
                D = D + T * dval;
            ...
            if (need_normal)
            {
                if (density_mode == SDF_MODE && has_sdf0 && !has_norm)
                {
                    // sdf0_t∏¶ æÓ∂≤ ∫πºøø° ¿˚øÎ«“¡ˆ ∞·¡§
                    bool use_next = false;
                    if (sdf0_t > ab.x && sdf0_t < ab.y) {
                        use_next = true;
                    }

                    // ≥Î∏ª ∞ËªÍ
                    float3 vox_c_for_grad = use_next ? vox_c : prev_vox_c;
                    float vox_l_for_grad  = use_next ? vox_l : prev_vox_l;
                    const float* g_for_grad = use_next ? geo_params : prev_geo_params;

                    float vox_l_inv = 1.f / vox_l_for_grad;
                    float3 p_sdf0 = ro + sdf0_t * rd;
                    float3 qt_sdf0 = (p_sdf0 - (vox_c_for_grad - 0.5f * vox_l_for_grad)) * vox_l_inv;

                    float x = qt_sdf0.x, y = qt_sdf0.y, z = qt_sdf0.z;

                    float grad_x =
                        (1 - y)*(1 - z)*(g_for_grad[1] - g_for_grad[0]) +
                        (    y)*(1 - z)*(g_for_grad[3] - g_for_grad[2]) +
                        (1 - y)*(    z)*(g_for_grad[5] - g_for_grad[4]) +
                        (    y)*(    z)*(g_for_grad[7] - g_for_grad[6]);

                    float grad_y =
                        (1 - x)*(1 - z)*(g_for_grad[2] - g_for_grad[0]) +
                        (    x)*(1 - z)*(g_for_grad[3] - g_for_grad[1]) +
                        (1 - x)*(    z)*(g_for_grad[6] - g_for_grad[4]) +
                        (    x)*(    z)*(g_for_grad[7] - g_for_grad[5]);

                    float grad_z =
                        (1 - x)*(1 - y)*(g_for_grad[4] - g_for_grad[0]) +
                        (    x)*(1 - y)*(g_for_grad[5] - g_for_grad[1]) +
                        (1 - x)*(    y)*(g_for_grad[6] - g_for_grad[2]) +
                        (    x)*(    y)*(g_for_grad[7] - g_for_grad[3]);
                    ...
                    /*
                    float3 p_sdf0 = ro + sdf0_t * rd;  // ?õî?ìú Ï¢åÌëú
                    float vox_l_inv = 1.f / vox_l;
    ...
    if (inside)
    {
        ...
        if (density_mode == SDF_MODE && has_sdf0)
            out_sdf0[pix_id] = sdf0_t;  // ray Î∞©Ìñ•?úºÎ°úÏùò Í±∞Î¶¨
        else
            out_sdf0[pix_id] = -1.f;
    ...
    const float* s_val)
{
    ...
    (density_mode == DENSITY_SDF) ?
        ...
        FwRendFunc(2, DENSITY_SDF); //Î¨¥Ï°∞Í±? ?ù¥?†áÍ≤?
