...
def parse_args():
    p = argparse.ArgumentParser(
        description="Run Pi3X export (conf/normal/poses/ply) then align points to NeRF GT (transforms_*.json) in one shot."
    )

    # --- Export stage args ---
    p.add_argument("--data_root", required=True, type=str,
                   help="Image directory, e.g. /.../dtu_scan122/images")
    ...
    # --- Alignment stage args (NeRF) ---
    p.add_argument("--gt_json", required=True, type=str,
                   help="Path to NeRF transforms_*.json (GT poses, OpenGL c2w). e.g. /.../transforms_train.json")
    p.add_argument("--out_ply", required=True, type=str,
                   help="Output path for merged aligned point cloud, e.g. /.../points3D.ply")
    p.add_argument("--sample_rate", type=float, default=1.0)
    p.add_argument("--name_filter", type=str, default=None,
                   help="Optional: keep only GT frames whose file_path contains this substring. Default: None.")
    ...
    # this should be the NeRF/JSON-based aligner (the one we wrote)
    p.add_argument("--align_script", type=str, default="scripts/pi3/nerf_pose_alignment.py")
...
def main():
    ...
    print("\n[OK] export + NeRF alignment finished successfully.")
